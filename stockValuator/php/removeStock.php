<?php
	...
	$con=mysqli_connect($hostname,$username,$password,$db); //Establishes connection with the database
				...
				$stock = $_POST['stock']; //Uses $_POST method to retrieve the stock id
				...
				$query = "DELETE FROM `Selected_Stocks` WHERE (user_id = $userId AND stock_id = $stock)"; //Removes stock selection with a user_id and stock_id matching the original parameters
				$result = mysqli_query($con, $query); //Queries the database
?>
